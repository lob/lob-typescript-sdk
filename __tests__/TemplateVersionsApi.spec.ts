...
describe("TemplateVersionsApi", () => {
  ...
  describe("performs template version operations", () => {
    ...
    it("creates, updates, retrieves, and deletes a template", async () => {
      const templateVersionsApi = new TemplateVersionsApi(config);
      // Create
      const createdTemplateVersion = await new TemplateVersionsApi(config).create(dummyTemplate.id as string, templateVersionWrite);
      expect(createdTemplateVersion?.id).toBeDefined();
      expect(createdTemplateVersion?.description).toEqual(templateVersionWrite.description);

      // Get
      const retrievedTemplateVersion = await templateVersionsApi.get(dummyTemplate.id as string, createdTemplateVersion.id as string);
      ...
      const updatedTemplateVersion = await templateVersionsApi.update(
        dummyTemplate.id as string, createdTemplateVersion.id as string,
        updatedVersion
      );
      ...
      // Delete
      const deletedTemplateVersion = await templateVersionsApi.delete(dummyTemplate.id as string, updatedTemplateVersion.id as string);
  ...
  describe("list templates", () => {
    ...
    beforeAll(async () => {
      ...
      const templateVersion2: TemplateVersionWritable = Object.assign({}, templateVersion1, {
        description: "Newer Template",
        html: "<html>Updated HTML for Template 2</html>"
      });
      const templateVersion3: TemplateVersionWritable = Object.assign({}, templateVersion1, {
        description: "Newer Template",
      ...
      const templateVersionsApi = new TemplateVersionsApi(config);
      await Promise.all([
        templateVersionsApi.create(dummyTemplate.id as string, templateVersion1),
        templateVersionsApi.create(dummyTemplate.id as string, templateVersion2),
        templateVersionsApi.create(dummyTemplate.id as string, templateVersion3),
      ])
        .then((creationResults) => {
          if (creationResults.length !== 3) {
            fail();
          }
          createdTemplateVersions = createdTemplateVersions.concat(creationResults);
        })
        .catch((err) => {
          fail(err);
        });
    ...
    afterAll(async () => {
      ...
      for (const templateVersion of createdTemplateVersions) {
        deleteOperations.push(templateVersionsApi.delete(dummyTemplate.id as string, templateVersion.id as string));
      }
    ...
    it("lists template versions", async () => {
      const response = await new TemplateVersionsApi(config).list(dummyTemplate.id as string);
    ...
    it("lists template versions given before or after params", async () => {
      ...
      const response = await new TemplateVersionsApi(config).list(dummyTemplate.id as string, 3);
      ...
      const responseBefore = await new TemplateVersionsApi(config).list(dummyTemplate.id as string, 10, before);
