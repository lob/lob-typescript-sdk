...
describe("USVerificationsApi", () => {
  ...
  describe("UsCsvVerificationsResponse", () => {
    it("exists", () => {
      ...
      expect(typeof verificationApi.UsCsvVerificationsResponse).toEqual(
        "function"
      );
    });
    ...
    it("handles errors returned by the api", async () => {
      ...
      try {
        await new USVerificationsApi(
          CONFIG_FOR_UNIT
        ).UsCsvVerificationsResponse(1, 2, 3, 4, 5, 6);
      } catch (err: any) {
    ...
    it("handles errors returned by the api with missing response.data", async () => {
      ...
      try {
        await new USVerificationsApi(
          CONFIG_FOR_UNIT
        ).UsCsvVerificationsResponse(undefined);
        fail("Should throw");
      } catch (err: any) {
        expect(err.message).toEqual("error");
      }
