...
describe("CardsApi", () => {
  ...
  describe("performs single-Letter operations", () => {
    it("all individual Letter functions exists", () => {
      expect(lettersApi.letterCreate).toBeDefined();
      expect(typeof lettersApi.letterCreate).toEqual("function");

      expect(lettersApi.letterRetrieve).toBeDefined();
      expect(typeof lettersApi.letterRetrieve).toEqual("function");

      expect(lettersApi.letterCancel).toBeDefined();
      expect(typeof lettersApi.letterCancel).toEqual("function");
    });

    it("creates, retrieves, and cancels a CERTIFIED letter", async () => {
      const letter = await lettersApi.letterCreate(certifiedLetter);
      expect(letter?.id).toBeDefined();
      expect(letter?.extra_service).toEqual("certified");
      if (letter?.id) {
        const retrievedLetter = await lettersApi.letterRetrieve(letter.id);
        expect(retrievedLetter).toBeDefined();
        const cancelledLetter = await lettersApi.letterCancel(letter.id);
    ...
    it("creates, retrieves, and cancels a REGISTERED letter", async () => {
      ...
      const letter = await lettersApi.letterCreate(registeredLetter);
      expect(letter?.id).toBeDefined();
